...
Tags: WordPress SSO, oauth 2.0, login, Single Sign-On, OpenId
...
**WordPress Single Sign-On** ( WordPress SSO ) with **OAuth & OpenID Connect** plugin allows unlimited sso / login ( Single Sign On ) with your **[Azure AD](https://plugins.miniorange.com/azure-ad-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Azure B2C](https://plugins.miniorange.com/azure-b2c-ad-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Office 365](https://plugins.miniorange.com/office-365-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[AWS Cognito](https://plugins.miniorange.com/aws-cognito-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Classlink](https://plugins.miniorange.com/classlink-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Discord](https://plugins.miniorange.com/discord-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Clever](https://plugins.miniorange.com/clever-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Ping](https://plugins.miniorange.com/ping-federate-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Keycloak](https://plugins.miniorange.com/keycloak-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[WHMCS](https://plugins.miniorange.com/whmcs-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Okta](https://plugins.miniorange.com/okta-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[WSO2](https://plugins.miniorange.com/wso2-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Identity Server](https://plugins.miniorange.com/identityserver4-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Onelogin](https://plugins.miniorange.com/onelogin-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Salesforce](https://plugins.miniorange.com/salesforce-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[G Suite / Google Apps](https://plugins.miniorange.com/google-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Invision Community](https://plugins.miniorange.com/invision-community-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Slack](https://plugins.miniorange.com/slack-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Amazon](https://plugins.miniorange.com/amazon-oauth-and-openid-connect-single-sign-on-sso-login)**, **[Twitter](https://plugins.miniorange.com/twitter-single-sign-on-wordpress-sso-oauth-openid-connect)**, **[Apple](https://plugins.miniorange.com/login-with-apple-app-using-wordpress-oauth-client)** or other custom OAuth 2.0, OpenID Connect providers. WordPress SSO ( Login ) plugin supports SSO with many OAuth 2.0, OAuth 2.1, OAuth 1.0 & OpenID Connect ( OIDC ) 1.0 providers.
It also provides unlimited User Authentication with OAuth & OpenID Connect protocol and allows authorized user to login into the WordPress site. Support provided for Single-site & Multisite Network environments.
 ...
 | <a href="https://plugins.miniorange.com/wordpress-single-sign-on-sso-with-oauth-openid-connect" target="_blank"> OAuth / OpenID Providers Setup guides </a>|<a href="https://youtu.be/Vff0E0KxM3k" target="_blank"> Videos </a>|<a href="https://plugins.miniorange.com/wordpress-sso#addons" target="_blank"> Addons </a>
...
WordPress Single Sign-On ( WordPress SSO ) allows users to login into any site / application using single set of credentials of another app / site.
...
Along with OAuth 2.0, the SSO plugin has also support for OAuth 1.0 and OAuth 2.1 protocol. Also, we support the latest OAuth and OpenID Connect protocol including secure 2-Factor Authentication, Multi-Factor Authentication if enabled at OAuth / OpenId provider's end.
...
**Cognito SSO**
This WordPress Single Sign-On ( OAuth / OpenID Connect SSO ) plugin supports Login with AWS Cognito, Login with Amazon, Login into WordPress with Cognito, and associated social login accounts. Also supports user profile syncing and role mapping into WordPress, fetching token from AWS Cognito to make the other API calls to extend the existing functionality.
...
**Ping Federate SSO**
WordPress SSO ( OAuth / OpenID Connect SSO ) plugin allows Login into WordPress using ping Federate. The plugin provides authorized access to different Ping REST APIs using the access token / bearer token.

**WHMCS SSO**
The WordPress SSO ( OAuth / OpenID Connect SSO ) plugin supports login with WHMCS to allow users ( clients, customers, workers, employees, technicians etc ) to access their WordPress portal based on their roles and capabilities. It also supports integration with the other Third Party plugins to sync the groups, courses, assignments, tasks with the WHMCS SSO application.
...
The plugin can also support WordPress Single Sign On with any Identity Provider including SAML, OAuth, OpenID Connect, Active Directory, database using miniOrange IDP allowing your users to login to the WP site via authenticating with their user store.
...
*	OpenAM / Forgerock ( supports OAuth 2.0 / OpenID Connect SSO for WordPress login )
...
*	<a href="https://plugins.miniorange.com/wordpress-hubspot-integration" target="_blank">**WordPress Hubspot Integration**</a> - WordPress HubSpot OAuth Integration allows you to integrate your WordPress website with the HubSpot platform or vice versa.
...
If you are looking for a OAuth / OpenID compliant Identity Provider, you can try out <a href="https://idp.miniorange.com">miniOrange On-Premise IdP</a>. Contact us at <a href="mailto:info@xecurify.com">info@xecurify.com</a> to get WordPress OAuth / OpenId connect add-ons.
...
= From WordPress.org =
1. Download WordPress OAuth Single Sign On – SSO (OAuth Client).
2. Unzip and upload the `miniorange-login-with-eve-online-google-facebook` directory to your `/wp-content/plugins/` directory.
3. Activate miniOrange SSO using OAuth from your Plugins page.
...
On configure OAuth page, check if your app is already there in default app list, if not then select the custom OAuth 2.0 app or Custom OpenID Connect Provider app based on the protocol supported by your provider. Then click on How to Configure link to see configuration instructions.
